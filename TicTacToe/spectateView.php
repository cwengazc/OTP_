...
$_SESSION['gamesessionid'] = 9; //Temporarily, choose active one for testing
...
$sql = "select * from ".$dbname.".player where id=".$pl1id;
$result = $conn->query($sql);
while($row = $result->fetch_assoc()){
	
	$pl1name= $row['username'];
}
$sql = "select * from ".$dbname.".player where id=".$pl2id;
$result = $conn->query($sql);
while($row = $result->fetch_assoc()){
...
<title><?php echo $_SESSION['player']['username'];?> - Tic Tac Toe</title>
