...
$sql = "SELECT * FROM ".$dbname.".`player` WHERE id =".$id;
...
if($result->num_rows>0){
	while($row = $result->fetch_assoc()){// check if the request is already sended.
	      if($row['senderid'] == $_SESSION['player']['id'] && $row['recieverid'] == $id){
          $i=1;
          break; 
		  }
	}
...
$sql = "INSERT INTO ".$dbname.".`requests`(`senderid`, `sendername`, `recieverid`, `recievername`, `status`)  VALUES (".$_SESSION['player']['id'].",'".$_SESSION['player']['username']."',".$id.",'".$name."',false)";
$conn->query($sql) ;
}
	$sql = "SELECT requestid FROM ".$dbname.".`requests` WHERE senderid=".$_SESSION['player']['id']." and recieverid=$id";// get the id of request send
	$result = $conn->query($sql);
	while($row = $result->fetch_assoc()){
...
<title><?php echo $_SESSION['player']['username'];?> - TicTacToe</title>
...
<nav class="navbar navbar-default">
  <div class="container-fluid"> 
    ...
    <a href="/homepage.php" > <button style="float:right; margin:2%;" class="btn btn-default"> <span class="glyphicon glyphicon-log-out"> </span> Quit</button></a>
